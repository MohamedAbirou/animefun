...
export default function AddEditGameModal({
  isOpen,
  onClose,
  game,
  onSuccess,
  ...
  useEffect(() => {
    if (game) {
      ...
      reset({ title: "", description: "", apk_file: "", tags: [] });
    }
  }, [game, reset]);

  const handleFileUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>, type: "screenshot") => {
      ...
      try {
        ...
        if (type === "screenshot") {
          // 1️⃣ Start compressing toast
          const compressToastId = toast.loading("Compressing screenshots...");

          const MAX_FILE_SIZE_MB = 1.2; // slightly more lenient than cover
          const MAX_FILE_SIZE_BYTES = MAX_FILE_SIZE_MB * 1024 * 1024;

          // Compress all large files in parallel
          ...
          const uploadToastId = toast.loading("Uploading screenshots...");
                ...
                >
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      {...register("apk_file", {
                        required: "APK file is required",
                      })}
                      className="form-input"
                    />
                    {errors.apk_file && (
                      <p className="mt-1 text-sm text-red-600">
                        {errors.apk_file.message}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {uploadProgress.screenshot > 0 &&
                      uploadProgress.screenshot < 100 && (
                        ...
                        {screenshots.map((path, index) => (
                          <div key={index} className="relative">
                            <img
                              ...
                              }/storage/v1/object/public/games/${path}`}
